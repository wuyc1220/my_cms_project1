/**
 * ObjectIngestHistoryModal — 关联对象注入历史弹框
 *
 * 用于 Cast/Category/Package/Picture 等关联对象的注入历史查看。
 * 通过 ingest_history_detail 表查询，展示 REGIST/UPDATE 动作记录。
 */
...
export default function ObjectIngestHistoryModal({
  open,
  entityType,
  entityId,
  entityName,
  ...
  const columns: ColumnsType<IngestHistoryDetailItem> = [
    ...
    },
    {
      ...
      render: (status: string | null) => {
        if (status === 'success') return <Tag color="success">{t('common.ingestStatus.success')}</Tag>
        if (status === 'failure') return <Tag color="error">{t('common.ingestStatus.failure')}</Tag>
