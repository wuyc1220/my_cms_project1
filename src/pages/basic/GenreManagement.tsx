...
export default function GenreManagement() {
  ...
  const [list, setList] = useState<GenreListItem[]>([])
  const [loading, setLoading] = useState(false)
  const [submitting, setSubmitting] = useState(false)
  const [modalOpen, setModalOpen] = useState(false)
  const [editingRecord, setEditingRecord] = useState<GenreListItem | null>(null)
  const [selectedIds, setSelectedIds] = useState<number[]>([])
  const [languageOptions, setLanguageOptions] = useState<LanguageOption[]>([])
  const { hasPermission } = usePermission()
  const canOperate = hasPermission('menu.basic.genres.operate')
  ...
  const searchFields: SearchFieldConfig[] = useMemo(() => [
    {
      name: 'name',
      labelKey: 'genre.form.nameLabel',
      type: 'input',
      placeholderKey: 'genre.form.nameRequired',
    },
    {
      name: 'languages',
      labelKey: 'genre.form.languageLabel',
      type: 'multiSelect',
      placeholderKey: 'genre.form.languageRequired',
  ...
  const openEdit = (record: GenreListItem) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (current) {
        const payload: GenreUpdatePayload = { name: values.name, language: values.language }
        await updateGenre(current.id, payload)
        closeModal()
        message.success(t('genre.msg.updated'))
      } else {
        const payload: GenreCreatePayload = { name: values.name, language: values.language }
        await createGenre(payload)
        closeModal()
        message.success(t('genre.msg.created'))
      }
  ...
  const handleDelete = async (record: GenreListItem) => {
  ...
  const handleBatchDelete = async () => {
    ...
    await batchDeleteGenres({ ids: selectedIds })
    message.success(t('common.msg.deleted'))
    setSelectedIds([])
    ...
    <div className="main-container">
      {/* 搜索区 */}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16, gap: 8 }}>
          ...
          {canOperate && (
            <Button type="primary" onClick={openCreate}>{t('genre.toolbar.newGenre')}</Button>
          )}
        </div>
      ...
      <Modal title={editingRecord ? t('genre.modal.titleEdit') : t('genre.modal.titleCreate')} open={modalOpen} onCancel={closeModal} onOk={() => void handleSubmit()} confirmLoading={submitting} okText={t('common.confirm')} cancelText={t('common.cancel')} destroyOnHidden>
        <Form form={itemForm} layout="vertical">
          <Form.Item name="name" label={t('genre.form.nameLabel')} rules={[{ required: true, message: t('genre.form.nameRequired') }, formRules.maxLength(FORM_MAX_LENGTH.INPUT)]}>
            <TrimInput placeholder={t('genre.form.nameRequired')} />
          </Form.Item>
          <Form.Item name="language" label={t('genre.form.languageLabel')} rules={[{ required: true, message: t('genre.form.languageRequired') }]}>
            <Select showSearch optionFilterProp="label" placeholder={t('genre.form.languageRequired')} options={languageOptions.map((item) => ({ label: item.name, value: item.code }))} />
          </Form.Item>
        </Form>
      </Modal>
    </div>
